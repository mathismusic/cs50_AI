...
class CrosswordCreator():
    ...
    def enforce_node_consistency(self): # YES!
        """
    ...
    def revise(self, x, y):
        ...
        if x != y:
            if self.crossword.overlaps[x, y] is None:  # notice square brackets in overlaps
                ...
                int_pt = self.crossword.overlaps[x, y]

        # if all((any((word1[int_pt[0]] == word2[int_pt[1]]) for word2 in self.domains[y])) for word1 in self.domain[x]):
        # the above is less syntax but we need explicity which words in x didn't work, so diff alg below

        Bool = False  # bool with lowercase b is a data type
        # check which words can give a possible y. other words won't give a solution
        for word in self.domains[x].copy():
            common_letter = word[int_pt[0]]
            if any((word2[int_pt[1]] == common_letter) for word2 in self.domains[y]):  # the variable name word can be used within any(.) again.
                continue
            else:
                Bool = True
    ...
    def ac3(self, arcs=None):
        ...
        """
        if arcs is None:
            # short for: for x: for y: if x != y: append((x, y)), for loops written L to R in the same way as while nesting loops
    ...
    def assignment_complete(self, assignment):
        ...
        """
        # check if all vars are assigned to something ie all vars are some or the other key, len works too, but this is less error-prone
        ...
        for word in self.domains[var]:
            ...
            for neighbor in (self.crossword.neighbors(var) - set(assignment)):  # neighbor is a Variable intersecting var
                ...
                for word2 in self.domains[neighbor]:
                    if word2[y] != char:  # this particular word in neighbor is ruled out if word is the value of var
                        count += 1
            list_of_tuples.append((count, word))
        sorted_list_of_tuples = sorted(list_of_tuples)  # sorted based on list[i][0] in inc order
        sorted_list_of_values = [elem[1] for elem in sorted_list_of_tuples]
    ...
    def select_unassigned_variable(self, assignment):
        ...
        if len(self.crossword.neighbors(var)) > maxDeg:
            best_vars_v2 = [var]
        elif len(self.crossword.neighbors(var)) == maxDeg:
            best_vars_v2.append(var)
        return random.choice(best_vars_v2)

    def inferences(self, assignment):
        domain_copy = self.domains.copy()
        for var in assignment:
            self.domains[var] = {assignment[var]}
        # set other_vars is useful as other_vars is a fixed set now, can be used in line 270. As set is unordered, it must be fixed while iterating
        # Alternately replace it with set(self.domains) - set(assignment).copy() in line 270.
        other_vars = set(self.domains) - set(assignment)
        arcs = [(x, y) for x in set(self.domains) for y in set(self.domains) if x != y]
        for v1 in other_vars:
            for v2 in other_vars:
                if v1 != v2:
                    arcs.remove((v1, v2))  # v2, v1 also taken care
        # now run ac3 with arcs, the arcs between vars in other_vars unaffected by assignment so still consistent, no need to recheck those arcs
